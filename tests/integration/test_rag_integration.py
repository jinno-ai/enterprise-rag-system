...
def test_rag_pipeline_end_to_end(temp_vector_db, sample_documents, mocker):
    ...
    mock_response.choices = [mocker.Mock(message=mocker.Mock(content="Mocked AI response"), finish_reason="stop")]
    ...
    mock_embedding.data = [mocker.Mock(embedding=[0.1] * 1536) for _ in range(len(sample_documents))]
    mocker.patch("openai.embeddings.create", return_value=mock_embedding)
...
def test_vector_db_operations(temp_vector_db, sample_documents, mocker):
    ...
    # Mock OpenAI
    mock_embedding = mocker.Mock()
    mock_embedding.data = [mocker.Mock(embedding=[0.1] * 1536) for _ in range(len(sample_documents) + 1)]
    mocker.patch("openai.embeddings.create", return_value=mock_embedding)
...
def test_retrieval_with_filters(mocker):
    ...
    # Mock OpenAI
    mock_embedding = mocker.Mock()
    mock_embedding.data = [mocker.Mock(embedding=[0.1] * 1536) for _ in range(4)]
    mocker.patch("openai.embeddings.create", return_value=mock_embedding)
