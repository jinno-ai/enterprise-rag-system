...
def test_rag_pipeline_end_to_end(mocker, temp_vector_db, sample_documents):
    ...
    # Mock OpenAI
    mocker.patch("app.core.embeddings.openai.embeddings.create", return_value=mocker.Mock(data=[mocker.Mock(embedding=[0.1] * 1536)]))
    mocker.patch("app.services.rag_pipeline.openai.chat.completions.create", return_value=mocker.Mock(
        choices=[mocker.Mock(message=mocker.Mock(content="Mocked answer"), finish_reason="stop")],
        usage=mocker.Mock(total_tokens=100)
    ))
...
def test_vector_db_operations(mocker, temp_vector_db, sample_documents):
    ...
    # Mock OpenAI
    mocker.patch("app.core.embeddings.openai.embeddings.create", return_value=mocker.Mock(data=[mocker.Mock(embedding=[0.1] * 1536) for _ in sample_documents]))

    # Initialize
    vector_db = get_vector_db(db_type="faiss", index_path=temp_vector_db)
    vector_db.create_index(dimension=1536) # Dummy dimension
    vector_db.connect()
...
def test_hybrid_retrieval(mocker, temp_vector_db, sample_documents):
    ...
    # Mock OpenAI
    mocker.patch("app.core.embeddings.openai.embeddings.create", return_value=mocker.Mock(data=[mocker.Mock(embedding=[0.1] * 1536) for _ in range(4)]))
