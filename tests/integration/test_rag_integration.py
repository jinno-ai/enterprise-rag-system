...
def test_vector_db_operations(mock_embed, temp_vector_db, sample_documents):
    ...
    # Setup mock
    mock_embed.return_value.data = [MagicMock(embedding=[0.1] * 1536) for _ in sample_documents]
    ...
    vector_db.upsert(
        ...
        metadata=[{**doc["metadata"], "text": doc["text"]} for doc in sample_documents]
    )
...
def test_hybrid_retrieval(mock_embed, temp_vector_db, sample_documents):
    ...
    vector_db.upsert(
        embeddings,
        [f"id_{i}" for i in range(len(texts))],
        [{**doc["metadata"], "text": doc["text"]} for doc in sample_documents]
    )
...
def test_batch_query(mock_chat, mock_embed):
    ...
    mock_chat.return_value.choices = [MagicMock(message=MagicMock(content="Test answer"), finish_reason="stop")]
...
def test_retrieval_with_filters(mock_embed):
    ...
    # Setup mock
    mock_embed.return_value.data = [MagicMock(embedding=[0.1] * 1536) for _ in range(3)]
    # Mocking for query embedding too
    mock_embed.return_value.data.append(MagicMock(embedding=[0.1] * 1536))
    ...
    vector_db.create_index(dimension=len(embeddings[0]))
    vector_db.upsert(embeddings, [f"id_{i}" for i in range(len(documents))], metadatas)
