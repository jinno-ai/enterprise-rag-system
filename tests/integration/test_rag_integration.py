...
def test_vector_db_operations(mocker, temp_vector_db, sample_documents):
    ...
    embedding_model.embed_texts = Mock(return_value=mock_embeddings)
    ...
    embedding_model.embed_query = Mock(return_value=[0.1] * 1536)
...
def test_hybrid_retrieval(mocker, temp_vector_db, sample_documents):
    ...
    # Index documents
    texts = [doc["text"] for doc in sample_documents]
    vector_db.add_documents(texts, mock_embeddings, [doc["metadata"] for doc in sample_documents])
...
def test_retrieval_with_filters(mocker):
    ...
    vector_db.add_documents(documents, mock_embeddings, metadatas)
