...
def test_rag_pipeline_end_to_end(temp_vector_db, sample_documents, mocker):
    ...
    retriever.build_bm25_index([
        Document(content=doc["text"], metadata=doc["metadata"])
...
def test_hybrid_retrieval(temp_vector_db, sample_documents, mocker):
    ...
    # Mock OpenAI
    mocker.patch("openai.embeddings.create", return_value=mocker.Mock(data=[mocker.Mock(embedding=[0.1]*1536) for _ in range(4)]))
...
def test_batch_query(mocker):
    ...
    from app.services.document_loader import Document
    retriever.build_bm25_index([Document(content="Test", metadata={})])
