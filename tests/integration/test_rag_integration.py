...
def test_rag_pipeline_end_to_end(mock_openai, temp_vector_db, sample_documents):
    ...
    mock_openai.embeddings.create.return_value.data = [
        Mock(embedding=[0.1] * 1536) for _ in range(len(sample_documents))
    ]
...
def test_vector_db_operations(mock_openai, temp_vector_db, sample_documents):
    ...
    # Mock embeddings
    mock_openai.embeddings.create.return_value.data = [
        Mock(embedding=[0.1] * 1536) for _ in range(len(sample_documents) + 1)
    ]
...
def test_retrieval_with_filters(mock_openai):
    ...
    # Mock embeddings
    mock_openai.embeddings.create.return_value.data = [
        Mock(embedding=[0.1] * 1536) for _ in range(4)
    ]
