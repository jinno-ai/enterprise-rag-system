...
## Issue 6: コストとレイテンシ最適化のための動的LLMルーティングの実装

**タイトル:** 【Phase 3.3】コストとレイテンシ最適化のための動的LLMルーティング（Model Router）の実装

**内容:**
クエリの複雑度や要求される品質に応じて、使用するLLMモデルを動的に切り替えるルーティング機能を実装します。

**背景:**
エンタープライズRAGシステムにおいて、すべてのクエリに高性能（高コスト・高レイテンシ）なモデル（例: GPT-4o）を使用するのは、運用コストと応答速度の面で非効率です。
単純な挨拶、事実確認、または定型的な質問に対しては軽量モデル（例: GPT-4o-mini, Llama-3-8B等）を割り当てることで、全体的なスループットを向上させ、コストを大幅に削減できます。

**タスク:**
- [ ] **Model Router モジュールの追加**: クエリを入力とし、最適なモデルを選択するロジックの実装。
- [ ] **RAGPipeline の拡張**: 複数のモデル設定（'smart_model', 'fast_model'）をサポートし、Router の判定結果に基づき動的に切り替え。
- [ ] **構成管理の更新**: `Settings` クラス（`app/core/config.py`）に複数モデルの定義と閾値設定を追加。
- [ ] **レスポンス・メタデータの強化**: 最終的にどのモデルが選択されたか、およびその判定理由を `RAGResponse` に含める。
