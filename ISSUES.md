...
## Issue 6: 学習に基づくランキング統合 (Learned Fusion) の導入

**タイトル:** 【Phase 1.2】Learned Fusion (LightGBM/XGBoost) による検索ランキング統合の高度化

**内容:**
ハイブリッド検索において、RRF等の単純な統合手法ではなく、機械学習（Learning to Rank）を用いた高度な統合モデルを導入します。これにより、クエリやドキュメントの特徴量に基づいた最適なランキングが可能になります。

**タスク:**
- [ ] 特徴量エンジニアリング（BM25スコア、ベクトル類似度等）の実装
- [ ] LightGBM または XGBoost を用いたランキングモデルの学習・推論基盤の構築
- [ ] `HybridRetriever` へのモデル統合
- [ ] オフライン評価指標（nDCG, MRR）による精度検証
