...
## Issue 6: 日本語形態素解析の導入によるBM25検索精度の改善とAPIルートの統合

**タイトル:** 日本語形態素解析の導入によるBM25検索精度の改善とAPIルートの統合

**内容:**
1. **日本語形態素解析の導入**: 現在の `app/services/retrieval.py` における BM25 の実装では、単純な正規表現 (`re.findall(r'\w+', ...)`) を用いてトークン化を行っています。これは英語には有効ですが、わかち書きのない日本語では単語を正しく分割できず、キーワード検索の精度が著しく低下します。日本語形態素解析器（Sudachi, MeCab, fugashi 等）を導入し、日本語ドキュメントに対しても高精度なキーワード検索を可能にする必要があります。
2. **APIルートの統合**: 現在 `app/main.py` ではモック実装である `app/api/routes/ingest.py` がマウントされていますが、より高機能な実装である `app/api/routes/documents.py` が既に存在します。こちらに移行し、ドキュメント管理機能をプロダクションレベルに引き上げる必要があります。

**タスク:**
- [ ] 日本語形態素解析ライブラリを `requirements.txt` に追加する
- [ ] `app/services/retrieval.py` の `HybridRetriever` で言語に応じたトークナイザーを選択・実行できるようにリファクタリングする
- [ ] `app/main.py` のルーター定義を `ingest.router` から `documents.router` に変更する
- [ ] 統合後のAPI動作確認テストを追加・実施する
