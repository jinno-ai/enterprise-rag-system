...
## Issue 6: Phase 1.1: セマンティックチャンキングの実装

**タイトル:** 文脈境界に基づくセマンティックチャンキングの導入

**内容:**
固定長のチャンキングではなく、ドキュメントの文脈的な境界（セマンティック・バウンダリ）に基づいてテキストを分割する機能を実装します。これにより、チャンク内での情報の断片化を防ぎ、検索精度を向上させます。

**タスク:**
- [ ] `app/services/document_loader.py` に `SemanticSplitter` クラスを追加する
- [ ] 埋め込みモデルを用いた文間の類似度計算ロジックを実装する
- [ ] 既存のインジェクションパイプラインと統合する
- [ ] チャンク品質の評価指標を定義し、従来手法と比較する
