...
## Issue 6: PII検出・匿名化ユーティリティのRAGパイプラインおよびAPIレイヤーへの統合

**タイトル:** PII検出・匿名化（Redaction）ユーティリティのRAGパイプラインおよびAPIレイヤーへの統合

**内容:**
現在、`app/core/pii.py` には個人情報（メール、クレジットカード、SSN、電話番号、IPv4）を検出・匿名化する `PIIDetector` が実装されていますが、アプリケーションのメイン処理フローやAPIレイヤーには統合されておらず、有効活用されていません。本番環境において個人情報の漏洩（LLM外部サービスへの不必要な送信、およびログやキャッシュへの生データ保存）を防止するため、この検出・匿名化機能を適切に各コンポーネントに統合する必要があります。

**タスク:**
- [ ] `app/core/config.py` に PII検出・匿名化の有効化フラグ (`ENABLE_PII_REDACTION: bool = False`) および一括置換用マスク文字列の構成設定を追加する
- [ ] RAGパイプライン (`app/services/rag_pipeline.py`) にて、LLMにクエリや検索されたコンテキストを送信する前に、PII検出・匿名化処理を適用できるようにする
- [ ] APIのリクエスト・レスポンスのロギング・監査ログ出力処理に PII匿名化フィルタを統合し、ログへのセンシティブデータ出力を防ぐ
- [ ] 統合された処理が設定フラグに従って正しく動作することを検証するための単体テストを新規作成する
