...
## Issue 6: PII (個人情報) の検出とマスキング機能のパイプライン統合

**タイトル:** `PIIDetector` を利用した個人情報の検出・マスキング機能のRAGパイプラインおよびAPI層への統合

**内容:**
`app/core/pii.py` に、各種個人情報（メールアドレス、クレジットカード番号、SSN、電話番号、IPアドレスなど）を正規表現および Luhn アルゴリズムで検出し、マスキング（赤字化/リダクション）する堅牢なユーティリティクラス `PIIDetector` がすでに実装されています。
しかし現在、この `PIIDetector` はログ出力、キャッシュ、あるいは LLM（OpenAI等）へのAPIリクエストにおいて一切使用されていません。企業のコンプライアンス要件（GDPR, CCPA, 個人情報保護法など）を満たし、LLM や外部ロギングサービスへの不要な機密情報の流出を自動的に防ぐため、RAGシステム全体にこの個人情報マスキング機能を適用する必要があります。

**タスク:**
- [ ] `app/core/config.py` に `ENABLE_PII_REDACTION` 設定（ブール値、デフォルト: `True`）を追加する
- [ ] `app/api/routes/query.py` などのクエリ受付エンドポイントで、ユーザーから送られた質問をログに出力する前に `PIIDetector.redact` を適用する
- [ ] `RAGPipeline` がLLMクライアント（`AsyncOpenAI`）を呼び出す前に、プロンプト内（特にユーザーからの入力に由来する部分）を任意でマスキングする機能を導入する
- [ ] PII検出時の監査ログ（検出した個人情報の件数やタイプなど）を出力する仕組みを整える
- [ ] 各種エンドポイントやサービス層において、個人情報マスキングが正しく行われているかを検証する単体テスト・結合テストを追加する
