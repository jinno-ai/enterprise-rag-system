...
**タイトル:** 日本語形態素解析の導入によるキーワード検索の改善

**内容:**
現在、`HybridRetriever` のキーワード検索（BM25）において、単純な正規表現 `\w+` によるトークナイズが行われています。これは日本語のように分かち書きをしない言語においては適切ではなく、検索精度が著しく低下する原因となります。SudachiやMeCabなどの形態素解析器を導入し、適切な日本語トークナイズを実現する必要があります。

**タスク:**
- [ ] 日本語形態素解析ライブラリ（`sudachipy`, `sudachidict_full` 等）をインストールする
- [ ] `app/services/retrieval.py` の `build_bm25_index` および `keyword_search` において、日本語トークナイザーを使用するように修正する
- [ ] 言語判定を行い、日本語の場合は形態素解析、それ以外は従来のトークナイズを行うように切り替えロジックを実装する
- [ ] 形態素解析器の辞書更新等のメンテナンス手順を確立する
