...
**タイトル:** 日本語テキストのための形態素解析器の導入

**内容:**
現在、`app/services/retrieval.py` の `HybridRetriever.build_bm25_index` におけるトークナイズ処理は、単純な正規表現 (`re.findall(r'\w+', ...)`) に依存しています。これは英語などの分かち書きを行う言語には有効ですが、日本語のような分かち書きを行わない言語では単語を正しく分割できず、キーワード検索（BM25）の精度が著しく低下します。

**タスク:**
- [ ] `Janome` や `SudachiPy` などの日本語形態素解析ライブラリを導入する
- [ ] `HybridRetriever` のトークナイズ処理を、言語を判定して適切なアナライザーを使用するように修正する
- [ ] 日本語ドキュメントを用いたBM25の検索精度の評価と検証を行う
