...
## Issue 10: 高度なドキュメント解析 (Unstructured.io / 表構造抽出) の導入とインジェスト拡張

**タイトル:** Unstructured.io または高度なドキュメントパーサーによる表構造・マルチフォーマット対応の統合

**内容:**
`EPIC_PLANNING.md` の Story 2.1 (高度なドキュメント取り込み) に基づき、現在の基本テキストチャンキングおよびPDF/TXT等の単純ローダーから、PDF・DOCX・PPTX等の複雑なレイアウトや表(Table)データを正確に抽出・チャンキングできる `Unstructured.io` 等の高度なドキュメントパーサーを `app/services/document_loader.py` に統合する必要があります。

**タスク:**
- [ ] `Unstructured.io` または専用ライブラリを用いたマルチフォーマット解析モジュールを実装する
- [ ] ドキュメント内の表 (Table) 構造を崩さずにテキスト・HTML/Markdown形式で抽出する処理を追加する
- [ ] 抽出されたメタデータおよび表データを構造化チャンクとして保存・ベクトルデータベースへ登録する仕組みを構築する
- [ ] ドキュメント取り込み処理のユニットテストおよび統合テストを追加する
