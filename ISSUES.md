...
## Issue 6: マルチエージェントRAGの実装

**タイトル:** 【Phase 6.2】LangGraphによるマルチエージェントRAGの実装と複雑な推論タスクへの対応

**内容:**
現状のRAGパイプラインは単一の検索・生成フローに限定されており、複雑な推論や多角的な情報の統合が必要な質問に対して限界があります。Phase 6.2に基づき、LangGraphを導入して複数の専門エージェント（検索、推論、検証など）が協調して動作するマルチエージェントRAGシステムを構築します。

**タスク:**
- [ ] LangGraphを用いたワークフロー定義基盤の実装
- [ ] 役割別のエージェント（Retrieval Agent, Reasoning Agent, Validation Agent）の構築
- [ ] 外部ツール（Web検索、計算機など）の統合
- [ ] マルチエージェントによる自己修正・再検索ループの実装
