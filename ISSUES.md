...
## Issue 6: ドキュメントの更新検知と自動同期（Sync）メカニズムの実装

**タイトル:** 【Phase 2.2】ドキュメントの更新検知と自動同期（Sync）メカニズムの実装

**内容:**
現在のインジェスト処理では、ドキュメントのコンテンツのハッシュ値を元に `doc_id` を生成しています。そのため、同じソースファイルの内容が更新された場合、新しい `doc_id` で追加登録され、古いバージョンのチャンクがベクトルデータベース内に残存してしまいます。これを防ぎ、常に最新のドキュメントのみが検索対象となるよう、更新検知と同期メカニズムを実装する必要があります。

**タスク:**
- [ ] `Document` クラスの `doc_id` 生成ロジックの見直し（ソースファイル単位での一貫性確保）
- [ ] `VectorDB` インターフェースへの `delete_by_source` メソッドの追加
- [ ] インジェストスクリプトにおける「既存ドキュメントの存在チェックと更新が必要な場合のみの処理」の実装
- [ ] メタデータへの `file_hash` または `last_updated` の保存
