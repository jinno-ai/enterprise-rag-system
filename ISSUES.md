...
## Issue 6: 検索精度の向上（日本語対応）とシステム実装の完成

**タイトル:** 検索精度の向上（日本語対応）とシステム実装の完成

**内容:**
現在のシステムには、日本語ドキュメントに対する検索精度の課題や、一部未実装のコア機能、およびモック化されたエンドポイントが残っています。これらを解決し、プロダクション品質のRAGシステムとして完成させます。

**タスク:**
- [ ] **日本語対応の強化**: チャンク分割およびBM25トークナイズに形態素解析エンジン（JanomeまたはSudachi）を導入し、日本語検索の精度を向上させる。
- [ ] **FAISS実装の改善**: `FAISSVectorDB.delete` メソッドを実装し、ドキュメントの削除に対応する。また、`search` メソッドで `filter_dict`（メタデータフィルタリング）が機能するように修正する。
- [ ] **API統合の完成**: `app/main.py` で使用されているモック用の `ingest` ルーターを、機能実装済みの `app/api/routes/documents.py` に置き換え、実稼働させる。
- [ ] **Rerankerの統合**: `ContextCompressor` または retrieval flow に、既に実装されている `Reranker` サービス（Cross-Encoder）を正式に組み込む。
