...
**内容:**
現在、`app/services/retrieval.py` の `HybridRetriever.build_bm25_index` において、ドキュメントのトークン化に単純な正規表現 (`re.findall(r'\\w+', ...)`) が使用されています。これは英語には有効ですが、分かち書きを行わない日本語のテキストでは単語を正しく抽出できず、キーワード検索（BM25）の精度が著しく低下します。

**タスク:**
- [ ] 日本語形態素解析ライブラリ（MeCab, Sudachi, または Janome）を `requirements.txt` に追加する。
- [ ] 日本語テキストを検知、またはデフォルトで日本語としてトークナイズするロジックを `HybridRetriever` に実装する。
- [ ] 日本語のストップワード除去機能を実装する。
