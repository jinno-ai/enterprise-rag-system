...
## Issue 6: 日本語形態素解析（Sudachi/MeCab）の導入による検索精度の向上

**タイトル:** 日本語形態素解析の導入によるハイブリッド検索の精度向上

**内容:**
現在、`app/services/retrieval.py` における BM25 インデックス作成およびクエリのトークナイズ処理は、単純な正規表現 (`\w+`) に依存しています。これはスペース区切りのない日本語テキストにおいては適切に機能せず、単語単位でのマッチングが困難なため、キーワード検索の精度が大幅に低下しています。

エンタープライズ用途での日本語ドキュメント活用を想定し、形態素解析器（SudachiPy, MeCab 等）を導入して適切な「わかち書き」を行う仕組みが必要です。

**タスク:**
- [ ] 日本語形態素解析ライブラリ（`SudachiPy` または `fugashi`）を `requirements.txt` に追加する
- [ ] `app/services/retrieval.py` の `HybridRetriever` クラスを拡張し、日本語を検知した場合に形態素解析を行うトークナイザーを実装する
- [ ] ストップワード（「の」「に」「は」等）の除去機能を実装する
- [ ] 日本語のテストデータを用いたキーワード検索のユニットテストを追加する
