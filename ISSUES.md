...
## Issue 6: FAISSにおけるメタデータフィルタリングの実装

**タイトル:** FAISSベクトルデータベースにおけるメタデータフィルタリング機能の追加

**内容:**
現在、`FAISSVectorDB.search` メソッドは `filter_dict` 引数を受け取りますが、実際の検索処理では無視されています。FAISSの `IndexFlat` などはネイティブでメタデータフィルタリングをサポートしていませんが、検索結果を取得した後にメタデータでフィルタリングする、あるいはIDでフィルタリングする仕組みを導入する必要があります。

**タスク:**
- [ ] `FAISSVectorDB.search` 内で `filter_dict` を使用したフィルタリングロジックを実装する
- [ ] メタデータフィルタリングが正しく機能することを確認する単体テストを追加する
- [ ] 必要に応じて、大量のドキュメントがある場合のパフォーマンスを考慮したフィルタリング方式（事前フィルタリング vs 事後フィルタリング）を検討する
