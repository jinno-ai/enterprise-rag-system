...
**内容:**
現在の `HybridRetriever` における BM25 インデックス作成時のトークナイズ処理は、正規表現 `\w+` を使用しており、日本語の文章を適切に分かち書き（形態素解析）することができません。これにより、日本語での検索精度が著しく低下しています。日本語の文章を適切に処理するために、Sudachi や MeCab などの形態素解析器を導入する必要があります。

**タスク:**
- [ ] `app/services/retrieval.py` の `build_bm25_index` メソッドに形態素解析ロジックを追加する
- [ ] 日本語対応のトークナイザーを選択可能にする設定を `config.py` に追加する
- [ ] 日本語のドキュメントを用いた検索精度の検証を行う
