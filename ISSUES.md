...
## Issue 6: マルチレイヤーキャッシュ (Redis) の導入

**タイトル:** パフォーマンス向上とコスト削減のためのRedisキャッシュの実装

**内容:**
現在、同一のクエリに対しても毎回LLMの呼び出しとベクトル検索が行われており、レイテンシとAPIコストの両面で非効率です。ロードマップ Phase 3.2 に基づき、Redisを使用したマルチレイヤーキャッシュを導入する必要があります。

**タスク:**
- [ ] Redis接続管理ユーティリティの実装
- [ ] クエリ結果（RAGResponse）をシリアライズしてキャッシュするロジックの追加
- [ ] キャッシュの有効期限（TTL）の設定と、ドキュメント更新時のキャッシュ無効化（Invalidation）戦略の実装
- [ ] セマンティックキャッシュ（類似クエリに対するキャッシュ）の検討と実装
