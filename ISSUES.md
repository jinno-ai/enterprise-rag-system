...
## Issue 6: 【検索基盤】BM25インデックスの永続化と再起動時の自動ロード機能の実装

**タイトル:** BM25インデックスのシリアライズと永続化

**内容:**
現在の `HybridRetriever` では、BM25インデックスがメモリ上にのみ保持されており、アプリケーションの再起動のたびに全ドキュメントを再スキャンしてインデックスを再構築する必要があります。大規模なデータセットでは起動時間の遅延やリソース消費が問題となるため、FAISSインデックスと同様にファイルベースでの保存・読み込み機能を実装する必要があります。

**タスク:**
- [ ] BM25インデックスのシリアライズ機能の実装
- [ ] インデックスのファイル保存/読み込みロジックの実装
- [ ] アプリケーション起動時に保存済みインデックスがあればロードする仕組みの導入
