...
## Issue 7: セキュリティとプライバシーの強化 (PII マスキング)

**タイトル:** PII（個人識別情報）の自動検知とLLM送信コンテキスト・ログのマスキング統合

**内容:**
`app/core/pii.py` に `PIIDetector` クラス（メールアドレス、クレジットカード番号、SSN、電話番号、IPアドレスの検知・マスキング機能）が実装されましたが、RAG パイプラインのコンテキスト構築処理や API のリクエスト/レスポンスログ出力等へは未統合です。

外部 LLM プロバイダーへの不要な個人情報送信を防止し、かつログファイルへの機密情報漏洩を防ぐため、`PIIDetector` を `RAGPipeline` およびロギング/ミドルウェア層へ統合する必要があります。

**タスク:**
- [ ] `RAGPipeline` 内のプロンプト構築およびコンテキスト圧縮処理に `PIIDetector.redact()` を組み込む（オプション設定可能化）
- [ ] リクエスト/レスポンスログ出力時に PII を検知・マスキングするロギングフィルタまたはミドルウェアを実装する
- [ ] PII マスキング統合に関する単体テストおよび結合テストを追加する
