...
## Issue 6: 日本語形態素解析の導入とドキュメント管理APIの正式統合

**タイトル:** 日本語形態素解析の導入とドキュメント管理APIの正式統合

**内容:**
現在、RAGシステムの日本語対応において以下の2点の課題があります。これらを解決し、実用的な日本語検索基盤を構築します。

1.  **BM25検索の精度不足:** `HybridRetriever` におけるトークナイズ処理が単純な正規表現（空白区切り想定）になっており、単語間に空白のない日本語ドキュメントではキーワード検索が正しく機能しません。JanomeやSudachiなどの形態素解析器を導入する必要があります。
2.  **モックAPIの残存:** `app/main.py` において、実際の機能を備えた `app/api/routes/documents.py` ではなく、モック実装である `app/api/routes/ingest.py` がマウントされています。これを正式な実装に差し替える必要があります。

**タスク:**
- [ ] `app/services/retrieval.py` に日本語トークナイザー（Janome等）を導入し、BM25インデックス作成を改善する。
- [ ] `app/main.py` のルーティングを `ingest.router` から `documents.router` に変更する。
- [ ] ドキュメント管理APIの統合テストを追加し、正常にインジェストと検索ができることを確認する。
