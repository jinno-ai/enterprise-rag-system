...
**タイトル:** ドキュメント管理APIの統合とインデックス永続化の修正

**内容:**
現在、アプリケーションには以下の不整合および未実装事項があります。
1. `app/main.py` で `ingest` ルーター（モック実装）が使用されており、`documents` ルーター（実体実装）が統合されていない。
2. `FAISSVectorDB` が保存時にメタデータを `pickle` で書き出しているが、読み込み時 (`connect`) にそれをロードしていない。
3. `HybridRetriever` の BM25 インデックスがメモリ上にのみ存在し、再起動時に消失する。また、起動時に既存ドキュメントから再構築されない。

**タスク:**
- [ ] `app/main.py` のルーターを `ingest` から `documents` へ差し替える。
- [ ] `FAISSVectorDB.connect` で `.metadata.pkl` が存在する場合に自動でロードするよう修正。
- [ ] BM25 インデックスの保存・読み込み機能を `HybridRetriever` に追加する。
- [ ] アプリケーション起動時に、保存されたドキュメントから BM25 インデックスを復元する処理を追加する。
