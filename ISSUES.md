...
**内容:**
現在、`app/api/routes/ingest.py` のインジェクション機能はモック実装となっています。大規模なドキュメント群の処理において、APIリクエスト内での同期処理はタイムアウトのリスクがあるため、ロードマップ Phase 3.1 に基づき非同期タスク処理を導入します。
（※ `run.py` とのインターフェース不一致は初期対応済みですが、タスク化に伴う再整理が必要です）

**タスク:**
- [ ] `Celery` および `Redis` を導入し、非同期タスク実行基盤を構築する
- [ ] インジェクション処理を非同期タスク化し、APIから進捗状況（ステータス、処理済み件数、エラー等）を確認可能にする
- [ ] `app/api/routes/ingest.py` のエンドポイントを非同期タスク起動・状態確認用に刷新する
