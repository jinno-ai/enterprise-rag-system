...
## Issue 6: BM25インデックスの永続化と同期の導入

**タイトル:** 【検索精度】BM25インデックスの永続化とベクトルDBとの同期

**内容:**
現状、`HybridRetriever`で使用しているBM25インデックスはメモリ上にのみ保持されており、アプリケーションの再起動時に消失します。FAISSインデックスと同様に、BM25インデックスもディスクに永続化し、起動時に自動ロード、およびドキュメントの追加・削除時にベクトルDBと同期をとる仕組みを導入する必要があります。

**タスク:**
- [ ] BM25インデックスのシリアライズ・デシリアライズ機能の実装
- [ ] アプリケーション起動時のBM25インデックス自動ロード機能の追加
- [ ] ドキュメントのインジェスト時にBM25インデックスを更新・保存するロジックの実装
- [ ] FAISSインデックスとの整合性チェック機能の実装
