...
## Issue 6: 高度なドキュメント取り込みと多形式フォーマットの解析対応

**タイトル:** Unstructured.io等を用いたPDF/DOCX/PPTX解析および表構造抽出機能の導入 (Story 2.1)

**内容:**
`app/services/document_loader.py` におけるドキュメント解析処理を拡張し、`Unstructured` ライブラリ等を導入することで、PDF・DOCX・PPTX ファイル内の複雑なレイアウトや表 (Table) データを正確に知識ベースへ取り込めるようにします。

**タスク:**
- [ ] `Unstructured` ライブラリを用いた DOCX/PPTX/PDF パーサーの実装
- [ ] 表 (Table) 構造を維持したテキスト抽出およびメタデータ付与機能の追加
- [ ] セマンティックチャンキング処理と連携したチャンク分割精度の向上
- [ ] 関連ユニットテストの追加と既存ドキュメントローダーの動作検証
