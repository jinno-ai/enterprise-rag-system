...
## Issue 6: FAISS インデックス保存時における排他ロック（File Locking）の導入

**タイトル:** 【整合性】FAISS インデックス保存時における排他ロック（File Locking）の導入

**内容:**
現在、FAISS インデックスの永続化処理において、ファイル書き込み時の排他制御（ロック機構）が実装されていません。マルチユーザー環境や非同期タスクによる同時書き込みが発生した場合、インデックスファイルおよびメタデータファイルが破損するリスクがあります。

**タスク:**
- [ ] `FAISSVectorDB.save` にファイルロック機構を導入する
- [ ] 一時ファイルへの書き込みと `os.replace` によるアトミックな置換を実装する
- [ ] 並行インジェスト時の競合テストケースを追加する
