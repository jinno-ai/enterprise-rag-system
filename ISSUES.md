...
## Issue 6: 個人識別情報（PII）の検出とマスキング機能の統合

**タイトル:** PII（個人識別情報）フィルターのロギング・RAGパイプライン統合とテスト自動化

**内容:**
現在、`app/core/pii.py` にメールアドレス、クレジットカード、SSN、電話番号、IPv4を検出・マスクする `PIIDetector` ユーティリティクラスが追加されていますが、アプリケーション全体（構造化ロギング、キャッシュ、外部LLMプロバイダーへのクエリ転送フロー、およびドキュメント取り込み処理など）にまだ統合されていません。本番運用の安全性を高めるため、クエリやドキュメントに個人情報が含まれる場合のリアルタイム検出、構造化ログ内での自動マスキング、および外部LLMにコンテキストを送信する直前でのマスキング処理を実装・設定できるようにする必要があります。

**タスク:**
- [ ] 設定クラス (`app/core/config.py`) に PII フィルタリングの有効化フラグ (例: `ENABLE_PII_REDACTION`) などの設定項目を追加する。
- [ ] 構造化ロギング (`app/core/logging_config.py` または `app/utils/logging.py`) に、リクエストやレスポンスに PII が含まれている場合にそれを検知・マスクして出力するプロセッサ・ヘルパーを組み込む。
- [ ] RAGパイプライン (`app/services/rag_pipeline.py`) にて、外部LLM (OpenAI等) にコンテキストやクエリを送信する直前に `PIIDetector.redact()` を呼び出す安全フィルター処理を実装する。
- [ ] 統合テスト (`tests/integration/test_rag_integration.py` 等) に、PIIが含まれる質問やドキュメントを送信した際に、正しくマスキング処理が行われ外部に漏洩しないかを検証するテストシナリオを追加する。
