...
## Issue 6: 日本語形態素解析の導入によるハイブリッド検索精度の向上

**タイトル:** 日本語形態素解析（Sudachi/MeCab）の導入によるBM25検索の精度改善

**内容:**
現在、`app/services/retrieval.py` の `HybridRetriever` におけるキーワード検索（BM25）のトークナイズ処理は、単純な正規表現 (`re.findall(r'\w+', ...)`) に依存しています。これは英語などの分かち書き言語には有効ですが、日本語においては単語の境界を正しく認識できず、検索精度が著しく低下する原因となります。

日本語 RAG システムとしての品質を向上させるため、形態素解析エンジンを導入し、適切な単語分割とストップワード除去を行う必要があります。

**タスク:**
- [ ] `SudachiPy` または `MeCab` を依存関係に追加する
- [ ] `HybridRetriever.build_bm25_index` および `keyword_search` 内のトークナイズ処理を形態素解析ベースにリファクタリングする
- [ ] 日本語特有のストップワード（助詞、助動詞など）を除外するロジックを実装する
- [ ] 日本語ドキュメントを用いた検索精度の検証テストを追加する
