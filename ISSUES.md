...
**内容:**
現在の HybridRetriever では、BM25インデックス作成時のトークン化に re.findall(r'\w+', ...) を使用しており、分かち書きのない日本語テキストでは適切なキーワード検索ができません。これを改善するために Sudachi や MeCab などの日本語形態素解析器を導入し、精度の高いトークン化を行う必要があります。

**タスク:**
- [ ] Sudachi または MeCab を依存関係に追加する
- [ ] HybridRetriever.build_bm25_index におけるトークナイザを形態素解析器に差し替える
