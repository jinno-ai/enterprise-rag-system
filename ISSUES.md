...
## Issue 6: ドキュメント・レベルのアクセス制御 (ACL) の実装

**タイトル:** 【Phase 2.1】ドキュメント・レベルのアクセス制御 (ACL) の実装による検索結果のフィルタリング

**内容:**
現在の RAG システムでは、検索時にすべてのドキュメントが対象となっており、ユーザーの権限に基づいた検索結果の制限（ドキュメント単位でのアクセス制御）が実装されていません。エンタープライズ環境では、機密情報を含むドキュメントに対して特定のユーザーやグループのみがアクセスできるように制御する必要があります。

**タスク:**
- [ ] ドキュメントのメタデータに `allowed_users` および `allowed_groups` フィールドを追加
- [ ] インジェストAPIにおける ACL 情報の受け取りと保存の実装
- [ ] `VectorDB` の `search` メソッドにおけるメタデータフィルタリングの活用（`filter_dict` の構築）
- [ ] `RAGPipeline` におけるクエリ実行時のユーザーコンテキスト（UserInfo）の統合
- [ ] アクセス権限がないドキュメントが検索結果に絶対に含まれないことの検証テストの実装
