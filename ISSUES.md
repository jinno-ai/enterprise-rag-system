...
## Issue 6: 個人情報保護 (PII) の検知およびマスキング機能の統合

**タイトル:** PIIDetectorのRAGパイプラインおよびリクエスト/レスポンスログへの統合

**内容:**
`app/core/pii.py` に単体テスト済みの `PIIDetector` クラスが追加されましたが、現時点では外部LLM呼び出し前のコンテキスト生成処理や、APIリクエスト/レスポンスのロギングミドルウェアに統合されていません。企業向けエンタープライズセキュリティ要求を満たすため、外部プロバイダへ個人情報（メールアドレス、クレジットカード番号、SSN、電話番号、IPアドレスなど）が送信されたり、ログファイルに生データが記録されたりするのを防ぐ仕組みが必要です。

**タスク:**
- [ ] `app/services/rag_pipeline.py` において、LLMプロンプト生成（`_build_prompt`）前または文脈圧縮時に `PIIDetector.redact()` を適用可能にするオプション設定（`enable_pii_redaction`）を追加する
- [ ] リクエスト/レスポンスの構造化ロギングミドルウェア（または `ValidationMiddleware`）に `PIIDetector.redact()` を組み込み、ログ出力時のセンシティブ情報をマスクする
- [ ] PIIマスキング動作の検証を行う単体テスト・結合テストを追加する
