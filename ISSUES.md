...
**内容:**
現在、`app/services/retrieval.py` の BM25 インデックス作成において、単純な正規表現 (`\w+`) によるトークナイズが行われています。日本語には単語区切りがないため、この手法では検索精度が著しく低下します。Sudachi や MeCab などの形態素解析器を導入し、適切なトークナイズを行う必要があります。

**タスク:**
- [ ] `rank-bm25` に渡すトークナイザーを日本語対応のものに変更する
- [ ] 必要に応じて SudachiPy などのライブラリを `requirements.txt` に追加する
- [ ] 日本語テキストでの検索精度を評価するテストケースを追加する
