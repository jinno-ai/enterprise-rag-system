...
**タイトル:** 日本語ドキュメントに対するBM25検索精度の向上

**内容:**
現在、`HybridRetriever.build_bm25_index` では、`re.findall(r'\w+', doc.content.lower())` を使用してトークナイズを行っています。これは空白区切りの言語には有効ですが、日本語のように語境界が明示されない言語では適切にキーワードを抽出できず、検索精度が著しく低下します。日本語の検索精度を向上させるために、形態素解析器（MeCab, Janome, Sudachiなど）を導入する必要があります。

**タスク:**
- [ ] 形態素解析ライブラリ（`janome` など）を依存関係に追加する
- [ ] `HybridRetriever` 内に言語判定または日本語用トークナイザーを実装する
- [ ] 日本語ドキュメントを用いたBM25検索のテストケースを追加する
