...
**タイトル:** 日本語検索精度の向上、ドキュメント管理APIの統合、およびメタデータフィルタリングの実装

**内容:**
現在のシステムには、日本語の検索精度やドキュメント管理機能において以下の技術的負債および改善の余地があります。これらを解消し、プロダクション環境での実用性を高める必要があります。

1. **日本語検索精度の向上:**
   - `TextSplitter` および `HybridRetriever` において、単純な文字ベースの分割や正規表現によるトークナイズが行われており、日本語の文脈を考慮した処理が不足しています。
   - 形態素解析ツール（JanomeやSudachi等）を導入し、適切なチャンキングとトークナイズを実装する必要があります。

2. **ドキュメント管理APIの統合:**
   - `app/main.py` で現在マウントされている `ingest.router` はモック実装です。これを、`app/api/routes/documents.py` で定義されている機能的なAPIに置き換える必要があります。

3. **FAISSメタデータフィルタリングの実装:**
   - `FAISSVectorDB.search` において `filter_dict` パラメータが定義されていますが、現在は無視されています。メタデータに基づくフィルタリング機能を実装する必要があります。

**タスク:**
- [ ] 日本語形態素解析器を `TextSplitter` に統合し、セマンティックなチャンキングを実現する。
- [ ] `HybridRetriever` のキーワード検索（BM25）に日本語トークナイザーを導入する。
- [ ] `app/main.py` の `ingest` ルーターを `documents` ルーターに置き換え、エンドポイントを整理する。
- [ ] `FAISSVectorDB` にリニアスキャン等によるメタデータフィルタリングロジックを実装する。
- [ ] 統合されたドキュメント管理機能のユニットテストおよび統合テストを追加する。
