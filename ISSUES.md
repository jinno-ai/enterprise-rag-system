...
## Issue 6: Redisによるクエリ結果のキャッシング機能の実装

**タイトル:** Redisによるクエリ結果のキャッシング機能の実装

**内容:**
現在、`RAGPipeline` はリクエストのたびに検索と生成を行っています。同一または類似のクエリに対してレスポンス時間を短縮し、APIコストを削減するために、Redisを使用したキャッシング層を導入する必要があります。

**タスク:**
- [ ] Redisクライアントのユーティリティクラスを作成する
- [ ] `RAGPipeline.query` メソッドにキャッシュのルックアップと保存のロジックを追加する
- [ ] キャッシュの有効期限（TTL）を `config.py` から制御できるようにする
- [ ] 新しいドキュメントがインジェストされた際に、関連するキャッシュを無効化（invalidate）する仕組みを検討・実装する
- [ ] `docker-compose.yml` にRedisサービスを追加する
