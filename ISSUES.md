...
## Issue 6: ドキュメント管理APIの統合とFAISSインデックス削除機能の実装

**タイトル:** ドキュメント管理ルーター（`/documents`）のメインアプリへの統合およびFAISSベクトルデーターベースにおけるドキュメント削除・再構築の実装

**内容:**
現在、`app/api/routes/documents.py` には高性能かつ高機能なドキュメント登録・管理用API（一括インジェスト、単一アップロード、非同期バッチ処理、統計取得など）が定義されています。しかし、`app/main.py` 内ではこのルーター（`/documents`）がマウントされておらず、代わりにモック実装のみの `/api/v1/ingest` エンドポイントを提供する `app/api/routes/ingest.py` がマウントされています。

また、ドキュメントのライフサイクル管理（削除機能）において、`FAISSVectorDB` クラス（`app/core/vectordb.py`）の `delete` メソッドは現在ログ出力のみのプレースホルダーであり、実際のデータ削除およびインデックス再構築処理が実装されていません。

本番運用に向けた完全なドキュメント管理ライフサイクルをサポートするために、ルーターの統合およびFAISS削除ロジックの具現化が必要です。

**タスク:**
- [ ] `app/main.py` において、モックの `ingest.router` に代わり、本質的な機能を持つ `documents.router` を `/api/v1` プレフィックス配下にマウントするようにリファクタリングする
- [ ] `FAISSVectorDB.delete` メソッドを実装する：
  - 指定された `ids` を除外した新しいインデックスおよびメタデータ、IDマッピング情報を再構築するロジックの実装
  - 削除後のインデックス保存（ディスクへの自動永続化）のサポート
- [ ] 統合された `documents` エンドポイントおよび `FAISSVectorDB` の削除ロジック、インデックス再構築を検証するためのテストコードを作成・強化する
- [ ] 既存のテストスイートおよびCI/CDパイプラインでの検証
