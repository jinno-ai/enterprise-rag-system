...
## Issue 6: ハイブリッド検索用BM25インデックスの永続化と動的更新の実装

**タイトル:** 【機能強化】ハイブリッド検索用BM25インデックスの永続化と動的更新の実装

**内容:**
現状の `HybridRetriever` では、キーワード検索に使用する BM25 インデックスがメモリ上にのみ保持されており、アプリケーションの再起動時にインデックスが失われます。また、ドキュメントの新規追加時にリトリーバーが保持するインデックスが自動的に更新されないため、最新のドキュメントが検索対象になりません。

**タスク:**
- [ ] `app/services/retrieval.py` に BM25 インデックスの保存・読み込みロジックを実装する
- [ ] `HybridRetriever` にドキュメントを動的に追加するメソッドを実装する
- [ ] `app/api/routes/documents.py` を `app/main.py` に統合し、インジェスト時にインデックスが更新されるようにする
- [ ] インデックスの永続化と動的更新に関するテストを追加する
