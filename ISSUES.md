...
## Issue 6: PII (個人情報) の検出・マスク機能のRAGパイプラインおよびAPI層への統合

**タイトル:** `PIIDetector` を用いた個人情報の検出とマスキング機能の統合

**内容:**
`app/core/pii.py` に、クレジットカード、SSN、電話番号、メールアドレス、IPv4 アドレスを検出・マスクする `PIIDetector` ユーティリティクラスが追加されていますが、現在この機能はどのエンドポイントやRAGパイプラインのフローにも統合されていません。

コンプライアンスやセキュリティの観点から、外部のLLMプロバイダー（OpenAIなど）に機密データや個人情報を送信する前に適切にマスクする機能、および構造化ログやAPIレスポンスの監査ログなどで個人情報を隠蔽する機能を実装する必要があります。

**タスク:**
- [ ] `app/core/config.py` に PII フィルタリングの有効化フラグ (`ENABLE_PII_REDACTION`) およびマスク用の設定値を追加する。
- [ ] `RAGPipeline` の `query` および `batch_query` 処理フローの中で、LLMプロバイダーへのプロンプト（ユーザーのクエリや検索されたドキュメントのコンテキスト）を送信する直前に `PIIDetector.redact` を適用する。
- [ ] APIリクエストとレスポンスをロギングする際、または構造化ログ出力モジュール (`app/core/logging_config.py` または関連ミドルウェア) 内で、ログメッセージ内の PII を自動的にマスクする仕組みを導入する。
- [ ] クエリのログや、管理用のデータベースなどに保存されるログに対しても、PII マスキングを適用できるようにする。
- [ ] 統合された PII 検出・マスク機能の動作を検証する単体テストおよび統合テストを実装する。
