...
## Issue 6: 高度なドキュメント取り込みパイプラインの実装とUnstructured.ioの統合

**タイトル:** PDF・Office文書からの表構造抽出およびセマンティックチャンキングの統合

**内容:**
現在、`app/services/document_loader.py` の `DocumentLoader` はシンプルな `pypdf` やテキスト読み込みのみに対応しており、複雑なレイアウト（表構造、DOCX、PPTX等）の適切な取り込みや文脈を考慮したセマンティックチャンキングが未実装です。RAGの回答精度向上のため、`Unstructured.io` などの高度なドキュメント解析ツールを統合し、構造化データ（表）や高度なファイルフォーマットに対応させる必要があります。

**タスク:**
- [ ] `DocumentLoader` に `Unstructured.io` / フォーマット別パーサー処理を統合する
- [ ] PDFやDocx内の表 (Table) データを破損・欠損させずにMarkdown形式等で抽出するロジックを実装する
- [ ] 文脈の切れ目を意識したセマンティックチャンキング処理を連携させる
- [ ] 対応フォーマット（DOCX, PPTX, PDF等）のローダー単体テスト・結合テストを追加する
