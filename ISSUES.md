...
## Issue 6: 個人識別情報 (PII) の検知とマスキング機能の統合

**タイトル:** ログ出力およびLLMプロンプト送信時のPII検知・マスキング機能の統合

**内容:**
現在、`app/core/pii.py` に `PIIDetector` クラス（個人識別情報の検知・置換ユーティリティ）が実装されていますが、アプリケーションの各機能（ログ出力、キャッシュ、LLMプロンプト送信）との統合が行われていません。
エンタープライズ品質のコンプライアンスおよびセキュリティ基準を達成するため、機密情報（メールアドレス、クレジットカード番号、社会保障番号、電話番号、IPアドレスなど）が外部のLLMプロバイダーに送信されたり、ログファイルにそのまま出力されたりするのを防ぐ仕組みを実装する必要があります。

**タスク:**
- [ ] `PIIDetector.redact()` を用いて、外部LLMサービス（OpenAI/Anthropicなど）に送信されるプロンプトテキストからPIIを自動的に検出・マスキングするインターセプターまたはミドルウェア/ラッパーを実装する
- [ ] 構造化ログ（`app/utils/logging.py` または `app/core/logging_config.py`）出力時に、リクエスト内容やレスポンスに含まれるPIIを自動的にマスキングするロギングフィルタまたはデコレータを実装する
- [ ] Redisなどのキャッシュへの保存時、あるいはデータベース保存前にPIIをフィルタリング・マスキングする仕組みを検討/実装する
- [ ] `app/core/pii.py` 自体の検知パターンの精度向上、および特定ドメイン固有のPIIパターン追加を可能にする設定（`settings` との連携）を追加する
- [ ] 統合されたPIIマスキングの挙動を検証するための単体テストおよび結合テストを追加する
