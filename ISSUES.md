...
**内容:**
`app/main.py` で `_rag_pipeline` というグローバル変性が使用されています。これはテスト時のモック化を困難にし、アプリケーションのステート管理を複雑にします。FastAPIのDependency Injectionシステムを活用すべきです。
...
## Issue 6: ドキュメント管理APIの統合とモックルーターの置き換え

**タイトル:** ドキュメント管理APIの統合とモックルーターの置き換え

**内容:**
現在 \`app/main.py\` では \`app/api/routes/ingest.py\` というモック実装のルーターが使用されています。一方で、実際にファイル読み込みやチャンク分割、ベクトルDBへの保存機能を備えた \`app/api/routes/documents.py\` が既に実装されていますが、アプリケーションに組み込まれていません。システムを実用的な状態にするために、これらのルーターを適切に置き換える必要があります。

**タスク:**
- [ ] \`app/main.py\` における \`ingest\` ルーターの登録を \`documents\` ルーターに変更する
- [ ] 既存の \`ingest.py\` を廃止するか、必要に応じて \`documents.py\` に統合する
- [ ] 統合後の API 動作確認テストを実施する
