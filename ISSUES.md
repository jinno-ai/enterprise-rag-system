...
**タイトル:** 日本語検索のための形態素解析エンジン（Sudachi/MeCab）の統合

**内容:**
現在、`HybridRetriever` の BM25 インデックス作成および検索におけるトークナイズ処理は、単純な正規表現（`\w+`）に基づいています。これは英語などの分かち書きを行う言語には有効ですが、日本語のように単語境界が明示されない言語では適切に機能せず、キーワード検索の精度が著しく低下します。日本語ドキュメントに対して高精度なハイブリッド検索を実現するために、Sudachi や MeCab などの形態素解析エンジンを導入する必要があります。

**タスク:**
- [ ] `requirements.txt` に形態素解析ライブラリ（`sudachipy`, `sudachidict_full` 等）を追加する
- [ ] 日本語トークナイザーの実装（`app/utils/tokenizer.py`）
- [ ] `HybridRetriever.build_bm25_index` および `keyword_search` で日本語トークナイザーを使用するように修正する
- [ ] 日本語ドキュメントを用いたキーワード検索の精度評価テストを追加する
