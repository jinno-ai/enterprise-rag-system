...
## Issue 6: FAISS インデックス読み込み時におけるメタデータストアの復元の実装

**タイトル:** FAISS インデックス読み込み時におけるメタデータストアの復元の実装

**内容:**
`app/core/vectordb.py` の `FAISSVectorDB` クラスにおいて、`save` メソッドは FAISS インデックスに加えてメタデータストアを `.metadata.pkl` として保存していますが、`connect` メソッドでは FAISS インデックスの読み込みのみが行われており、メタデータストアの復元が実装されていません。これにより、再起動後に検索を行っても本文やメタデータが取得できない問題が発生します。

**タスク:**
- [ ] `FAISSVectorDB.connect()` メソッド内で `.metadata.pkl` ファイルの存在を確認する
- [ ] 存在する場合、`pickle` を使用して `metadata_store`, `id_to_idx`, `idx_to_id` を読み込み、インスタンス変数にセットする
