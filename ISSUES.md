...
**内容:**
現在のBM25実装（`app/services/retrieval.py`）では、単純な正規表現（`\w+`）によるトークナイズが行われています。これは日本語のように語境界が明示されない言語では適切なインデックスが作成されず、検索精度が著しく低くなります。SudachiPyやJanomeなどの形態素解析器を導入し、適切な分かち書きを行う必要があります。

**タスク:**
- [ ] `sudachipy` および `sudachidict_core` の導入
- [ ] `HybridRetriever.build_bm25_index` におけるトークナイザの日本語対応
- [ ] 日本語ドキュメントを用いた検索精度の検証とテストの追加
