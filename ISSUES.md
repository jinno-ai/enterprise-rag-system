...
**タイトル:** 日本語ドキュメントに対する形態素解析ベースのトークナイズの実装

**内容:**
現在、`app/services/retrieval.py` の `HybridRetriever.build_bm25_index` において、`re.findall(r'\w+', ...)` を用いた単純な正規表現によるトークナイズが行われています。これは英語などのスペース区切りの言語には有効ですが、分かち書きを行わない日本語では適切な単語分割ができず、キーワード検索（BM25）の精度が著しく低下します。

**タスク:**
- [ ] 日本語形態素解析ライブラリ（MeCab, Sudachi, または Janome）を導入する
- [ ] `HybridRetriever` に日本語用のトークナイザーを実装し、言語設定に応じて切り替え可能にする
- [ ] 辞書の導入により、ドメイン固有語（専門用語、社内用語）の抽出精度を向上させる
