...
**タイトル:** 日本語形態素解析（Janome/Sudachi等）の導入によるBM25検索の改善

**内容:**
現在、`HybridRetriever.build_bm25_index` におけるトークナイゼーションは単純な正規表現 (`re.findall(r'\w+', ...)`) を使用しています。これは英語のような単語がスペースで区切られた言語には有効ですが、日本語のような単語間にスペースがない言語では、適切な単語単位での分割ができず、BM25によるキーワード検索の精度が著しく低下します。

エンタープライズRAGとして日本語ドキュメントを扱う場合、MeCab、Sudachi、Janomeなどの形態素解析器を導入し、適切な分かち書きを行う必要があります。

**タスク:**
- [ ] 形態素解析ライブラリ（Janome, Sudachi, MeCabのいずれか）を `requirements.txt` に追加する
- [ ] `app/services/retrieval.py` の `build_bm25_index` および `keyword_search` をリファクタリングし、日本語対応のトークナイザーを導入する
- [ ] 言語判定ロジックまたは設定によるトークナイザーの切り替えを実装する
- [ ] 日本語テキストを用いた検索精度の評価テストを追加する
