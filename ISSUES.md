...
**タイトル:** 日本語トークナイズのサポートによる検索精度の向上

**内容:**
現在の `HybridRetriever` における BM25 インデックス作成およびキーワード検索では、単純な正規表現 (`\\w+`) を用いたトークナイズが行われています。これは日本語のような分かち書きをしない言語においては正しく機能せず、検索精度が著しく低下します。

MeCab や Sudachi などの形態素解析器、あるいは `fugashi` / `ipadic` などを導入し、日本語テキストを適切にトークナイズできるように修正する必要があります。

**タスク:**
- [ ] `app/services/retrieval.py` のトークナイズ処理を修正する
- [ ] 日本語対応のトークナイザーを導入する (例: MeCab, Janome, or spaCy)
- [ ] 日本語テキストを用いた BM25 検索のテストケースを追加する
