...
## Issue 7: 日本語全文検索（BM25）の精度向上に向けた形態素解析の導入

**タイトル:** 日本語全文検索（BM25）の精度向上に向けた形態素解析の導入

**内容:**
現在、`HybridRetriever.build_bm25_index` におけるトークナイズ処理は単純な正規表現（`re.findall(r'\w+', ...)`）に基づいています。これは日本語においては単語の境界を正しく認識できず、検索精度が著しく低下する要因となっています。

**タスク:**
- [ ] 形態素解析ライブラリ（MeCab, Sudachi等）の選定と導入
- [ ] `HybridRetriever` における日本語トークナイズの実装
- [ ] 助詞・助動詞などのストップワード除去の実装
- [ ] 日本語検索精度の検証用テストの作成
