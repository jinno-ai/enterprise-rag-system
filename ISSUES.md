...
**タイトル:** FAISSVectorDBにおけるメタデータに基づく検索結果の絞り込み機能の実装

**内容:**
現在、`FAISSVectorDB.search` メソッドには `filter_dict` 引数が存在しますが、その実装は空であり、検索時にメタデータによる絞り込みが行われません。マルチテナント環境や、特定のカテゴリ（例：ドキュメント種別、日付）に限定した検索を可能にするため、FAISS検索結果に対してメタデータフィルタリングを適用する機能を実装する必要があります。

**タスク:**
- [ ] `FAISSVectorDB.search` メソッドでの `filter_dict` の処理ロジックの実装（後フィルタリングまたはIDSelectorの活用）
- [ ] フィルタリング機能のユニットテストの追加
- [ ] フィルタリングによる精度と検索速度への影響の評価
