...
## Issue 6: 日本語形態素解析器の導入による日本語RAG検索精度の向上

**タイトル:** 日本語形態素解析器（JanomeまたはSudachi）の導入と日本語RAG検索精度の向上

**内容:**
現在、`TextSplitter`（`app/services/document_loader.py`）は単純な文字区切りや改行でのチャンク分割を使用しており、また `HybridRetriever` の BM25 インデックス（`app/services/retrieval.py`）は単純な正規表現（`re.findall(r'\w+', text)`）による単語切り出しを採用しています。これらは英語の文章には適していますが、単語間にスペース（空白）を持たない日本語のテキストに対しては、不正確なチャンク分割や単語（トークン）の不適切な切り出しを引き起こし、検索漏れや適合率の低下、RAGの回答精度の悪化を招いています。

日本語コンテンツの取り込みおよび検索精度をエンタープライズ品質に向上させるため、日本語形態素解析器（Janome、Sudachi、またはMecabのPythonバインディング等）を導入し、適切な日本語の形態素単位でのチャンク分割（セマンティック・文脈指向チャンキングの強化）およびBM25向けの日本語トークナイザーの実装を行う必要があります。

**タスク:**
- [ ] `requirements.txt` / `requirements-test.txt` に日本語形態素解析ライブラリ（例: `janome` または `sudachipy`）を追加する。
- [ ] 日本語専用のテキストスプリッター（例: 文節や文脈の区切りを形態素解析で考慮したチャンキング処理）を `app/services/document_loader.py` に実装または統合する。
- [ ] `app/services/retrieval.py` の `build_bm25_index` およびクエリトークン化ロジックに、日本語形態素解析を用いたトークナイザーを導入する（日本語以外のテキストの場合は従来の正規表現/スペースベースにフォールバックする仕組みを持つ）。
- [ ] 日本語トークナイザーおよび日本語チャンキング処理の単体テストを `tests/unit/test_japanese_support.py` として実装する。
- [ ] 日本語の質問・ドキュメントセットを用いたRAG統合検索テストを作成し、検索精度が向上していることを検証する。
