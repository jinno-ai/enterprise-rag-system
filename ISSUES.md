...
## Issue 6: FAISSの機能強化（データ削除・メタデータフィルタリング）とドキュメント管理APIの統合

**タイトル:** FAISSの機能強化（データ削除・メタデータフィルタリング）とドキュメント管理APIの統合

**内容:**
現在、`FAISSVectorDB` の `delete` メソッドがプレースホルダー状態になっており、ドキュメントの物理的な削除（ベクトル削除）がサポートされていません。また、`search` メソッドの `filter_dict` 引数が実装上で無視されており、メタデータフィルタリングが機能していません。さらに、`app/api/routes/documents.py` には高性能なドキュメント・バッチ管理APIが定義されていますが、`app/main.py` でマウントされているのはモック実装の `ingest.py` です。これらを本番利用に耐えうる形へと改修・統合する必要があります。

**タスク:**
- [ ] `FAISSVectorDB.delete` を実装し、メモリ上およびディスク上のインデックス再構築・更新を可能にする
- [ ] `FAISSVectorDB.search` で `filter_dict` によるメタデータフィルタリングを実装する
- [ ] `app/main.py` にて、モック実装の `/api/v1/ingest` から、本機能の備わった `/api/v1/documents` ルーターへの移行・マウントを行う
- [ ] 追加機能に対応する単体テストおよび統合テストを作成・更新する
