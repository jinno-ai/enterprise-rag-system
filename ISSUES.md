...
**タイトル:** 日本語ドキュメントへの対応（トークナイザーの導入）

**内容:**
現在の `HybridRetriever` における BM25 インデックス作成時のトークナイズ処理は、正規表現 `\w+` を使用した簡易的なものです。これは英語などの分かち書きを行う言語には有効ですが、日本語のように語句の境界が明示されない言語では適切にキーワードを抽出できず、検索精度が著しく低下します。

**タスク:**
- [ ] 日本語形態素解析ライブラリの選定と導入（例: `SudachiPy`）
- [ ] `app/services/retrieval.py` の `build_bm25_index` および `keyword_search` におけるトークナイズ処理を修正
- [ ] 日本語ドキュメントを用いた検索精度の評価テストの追加
