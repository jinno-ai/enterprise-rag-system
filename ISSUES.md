...
## Issue 6: セマンティックチャンキング (Semantic Chunking) の導入

**タイトル:** 【Phase 1.1】セマンティックチャンキングの実装による分割品質の向上

**内容:**
Roadmap Phase 1.1 に基づき、セマンティックチャンキングを実装してドキュメント分割の品質を向上させます。現在の `TextSplitter` は固定文字数や改行による単純な分割を行っていますが、文脈の連続性を考慮した埋め込みベースの分割を導入することで、検索精度のさらなる向上を目指します。

**タスク:**
- [ ] 埋め込みモデルを利用した文間の類似度計算ロジックの実装
- [ ] 類似度の閾値に基づく動的なチャンク境界の決定機能
- [ ] 文の統計的な特性（パーセンタイル等）を利用した分割アルゴリズムの導入
- [ ] `TextSplitter` クラスへの統合と既存機能との互換性確保
- [ ] 分割品質の検証とパラメータ調整
