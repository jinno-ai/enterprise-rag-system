...
**タイトル:** 日本語環境におけるBM25検索精度の向上

**内容:**
現在、`app/services/retrieval.py` の `HybridRetriever` における BM25 のインデックス作成と検索では、単純な正規表現 `\w+` によるトークナイズが使用されています。これは英語などのスペース区切り言語には有効ですが、単語境界のない日本語では適切に機能しません。日本語ドキュメントに対して高精度なキーワード検索を実現するためには、形態素解析器の導入が必要です。

**タスク:**
- [ ] 日本語トークナイザー（MeCab, Sudachi, または fugashi + ipadic 等）を導入する。
- [ ] `HybridRetriever.build_bm25_index` および `keyword_search` メソッドを修正し、日本語トークナイズに対応させる。
- [ ] 必要に応じて、言語判定ロジックを追加し、多言語対応を強化する。
