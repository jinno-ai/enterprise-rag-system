...
## Issue 6: 【Phase 3.1】非同期ジョブキュー（Celery/Redis）によるドキュメント処理のバックグラウンド化

**タイトル:** 非同期ジョブキュー（Celery/Redis）によるドキュメント処理のバックグラウンド化

**内容:**
現在、ドキュメントのインジェストAPI（`app/api/routes/ingest.py`）はモック実装となっており、CLI（`run.py`）とサービス間での同期も不十分です。大量のドキュメントを処理する際、APIをブロックせずに効率的に処理を行うため、CeleryとRedisを用いた非同期タスクキューを導入する必要があります。

**タスク:**
- [ ] CeleryとRedisをプロジェクトに統合する
- [ ] インジェスト処理をバックグラウンドタスクとして実行するようにリファクタリングする
- [ ] タスクのステータス確認および進捗管理用エンドポイントの実装
- [ ] バックグラウンドでのインジェスト完了後、RAGパイプラインがインデックスを同期（リロード）する仕組みの構築
