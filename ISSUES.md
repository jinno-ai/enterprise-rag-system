...
**タイトル:** Cross-Encoder (BGE-Reranker) の導入による検索適合率の向上

**内容:**
現在、ハイブリッド検索の結果を RRF で融合していますが、最終的な精度向上のためには、上位 N 件の候補に対して Cross-Encoder を用いたリランキングを行うことが効果的です。

**タスク:**
- [ ] `Reranker` クラスのインターフェース設計と実装
- [ ] `BGE-Reranker` や `Cohere Rerank` 等の統合
- [ ] `HybridRetriever` の検索フローにリランキングステップを追加

---

## Issue 7: 【Phase 1.1】セマンティックチャンキングの実装による検索精度の向上

**タイトル:** セマンティックチャンキングの導入によるチャンク境界の最適化

**内容:**
現在の TextSplitter は固定長や単純な改行で分割していますが、文脈が途切れることがあります。埋め込みモデルを用いて文章間の意味的な連続性を評価し、最適な境界で分割する手法を導入します。

**タスク:**
- [ ] `SemanticTextSplitter` の実装
- [ ] 文章間のコサイン類似度に基づいた分割ロジックの構築
- [ ] 既存の `DocumentLoader` との統合
