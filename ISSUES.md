...
## Issue 6: 【整合性】ドキュメント管理APIの実装統合とBM25インデックスの永続化

**タイトル:** ドキュメント管理APIの正規実装への切り替えとBM25インデックスの永続化

**内容:**
現在の実装には、システム全体の整合性と永続性に関する以下の2つの大きな課題があります。

1. **APIルーターの不整合**: `app/main.py` で `app.api.routes.ingest`（モック実装）が使用されており、実際に実装されている `app.api.routes.documents`（正規実装）が統合されていません。
2. **BM25インデックスの非永続性**: `HybridRetriever` で使用されるキーワード検索用の BM25 インデックスがメモリ上のみに保持されており、アプリケーションの再起動時に消失します。これにより、再起動後は手動でインデックスを再構築するまでハイブリッド検索が機能しません。

**タスク:**
- [ ] `app/main.py` のルーター設定を修正し、`documents.py` のエンドポイントを有効化する
- [ ] `HybridRetriever` に BM25 インデックスの `save` および `load` メソッドを実装する
- [ ] 起動時のコンポーネント初期化フローで BM25 インデックスの自動ロードを組み込む
