...
## Issue 6: BM25インデックスの永続化と起動時の自動ロード

**タイトル:** 【Phase 1.2】BM25インデックスの永続化と起動時の自動ロードの実装

**内容:**
現在、`HybridRetriever` における BM25 インデックスはメモリ上でのみ構築されており、アプリケーションの再起動時に消失します。また、ドキュメントのインジェスト時にベクトルインデックス（FAISS）は保存されますが、BM25 インデックスは更新・保存されていません。ハイブリッド検索の精度を維持するためには、BM25 インデックスをディスクに永続化し、起動時に自動的にロードする仕組みが必要です。

**タスク:**
- [ ] `HybridRetriever` に BM25 インデックスを保存・読み込みするための `save_bm25_index` および `load_bm25_index` メソッドを実装する
- [ ] `app/api/routes/documents.py` のインジェスト処理後に BM25 インデックスを保存するように修正する
- [ ] `app/main.py` の起動処理（lifespan）において、保存済みの BM25 インデックスが存在する場合は自動的にロードするように実装する
- [ ] インデックスの保存形式（例：pickle や json）を検討し、ベクトルインデックスと同じディレクトリに配置する

**期待される結果:**
- サーバーを再起動してもキーワード検索（BM25）が機能し続けること
- 新規ドキュメント追加時にキーワード検索対象も正しく更新されること
