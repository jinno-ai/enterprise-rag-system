...
## Issue 6: ハルシネーション検知（Hallucination Detection）と自己修復ロジックの導入

**タイトル:** 【Phase 1.3】ハルシネーション検知（Hallucination Detection）と自己修復ロジックの導入による信頼性の向上

**内容:**
LLMが提供されたコンテキストを無視して回答を生成したり、虚偽の情報を混ぜたりするリスク（ハルシネーション）を低減するため、生成された回答の妥当性を検証するステップをパイプラインに追加します。現在の `_calculate_confidence` はスコアに基づいた単純なヒューリスティックに依存しており、内容の事実関係を直接検証していません。

**タスク:**
- [ ] 生成された回答がコンテキスト内の情報のみに基づいているかを判定する LLM 検証プロンプトの実装
- [ ] `Self-Correction` ループの導入（検証に失敗した場合、修正指示を添えて再生成）
- [ ] 低信頼度時や検証失敗時のフォールバック応答（「コンテキストから回答を生成できません」等）の強化
- [ ] 検証結果（Faithfulnessスコアなど）を `RAGResponse` のメタデータに含めるよう拡張
