...
## Issue 6: 日本語テキスト処理の精度向上

**タイトル:** 日本語形態素解析器の導入によるテキスト分割とBM25インデックスの改善

**内容:**
現在、`TextSplitter` や `HybridRetriever` の BM25 インデックス作成ロジックではシンプルな正規表現や文字ベースの区切り文字が使用されています。日本語ドキュメントに対して高精度な検索・分割を実現するために、Janome や SudachiPy などの日本語形態素解析器を導入する必要があります。

**タスク:**
- [ ] 日本語形態素解析ライブラリ（Janome / SudachiPy等）の選定と依存関係追加
- [ ] `TextSplitter` における日本語対応のチャンキングロジック実装
- [ ] `HybridRetriever` の BM25 トークナイザを形態素解析ベースに変更
- [ ] 日本語ドキュメントに対する精度検証およびテストコードの作成

---

## Issue 7: PII検出・マスキング機能のパイプラインおよびロギング/キャッシュへの統合

**タイトル:** `PIIDetector` の RAG コンテキスト生成・ログ・キャッシュ処理への統合

**内容:**
`app/core/pii.py` に個人情報（PII）の検出およびマスキングを行う `PIIDetector` ユーティリティが追加されましたが、現在 RAG パイプライン（LLMプロンプト送信前）、リクエスト/レスポンスログ出力時、キャッシュ保存処理に組み込まれておらず、個人情報が外部プロバイダへ送信されたりログに記録されるリスクが存在します。

**タスク:**
- [ ] RAGコンテキスト圧縮/プロンプト生成時に `PIIDetector.redact()` を適用し、LLMプロンプトへ送信される個人情報をマスクするオプションを追加する
- [ ] リクエスト/レスポンスログ出力時に PII を検知・マスクするミドルウェアまたはロガーフックを実装・統合する
- [ ] キャッシュ保存前のコンテキスト/レスポンスに対する PII リダクションの適用
- [ ] PII統合処理に関する単体テストおよび結合テストを追加する
