...
**タイトル:** [Retriever] 日本語形態素解析の導入によるBM25検索精度の改善

**内容:**
現在、`app/services/retrieval.py` の `HybridRetriever.build_bm25_index` において、ドキュメントのトークナイズに単純な正規表現 (`re.findall(r'\w+', ...)`) が使用されています。これは日本語のような分かち書きをしない言語においては不適切であり、キーワード検索の精度が低くなる原因となります。

**タスク:**
- [ ] 日本語形態素解析器（Janomeなど）を導入し、適切な単語分割を行えるようにする
- [ ] `HybridRetriever` を修正し、日本語ドキュメントに対して精度の高いBM25インデックスを作成・検索できるようにする
- [ ] 日本語ドキュメントを用いた検索精度の評価テストを追加する
