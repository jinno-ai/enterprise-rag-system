...
**タイトル:** 日本語形態素解析の導入によるキーワード検索（BM25）精度の向上

**内容:**
現在、`app/services/retrieval.py` における BM25 用のトークナイズ処理では、単純な正規表現 (`re.findall(r'\w+', ...)`) が使用されています。これは英語などの分かち書きを行う言語には適していますが、日本語のように分かち書きを行わない言語では単語を正しく分割できず、キーワード検索の精度が著しく低下します。

エンタープライズ品質の検索を実現するために、日本語形態素解析器（SudachiPy, MeCab等）を導入し、日本語を適切にトークナイズする必要があります。

**タスク:**
- [ ] 日本語形態素解析ライブラリ（例: `SudachiPy`, `sudachidict_core`）を `requirements.txt` に追加する
- [ ] `app/services/retrieval.py` の `build_bm25_index` メソッドを修正し、日本語の形態素解析に対応させる
- [ ] `keyword_search` メソッドのクエリトークナイズ処理にも形態素解析を適用する
- [ ] 日本語のストップワード除去処理を実装する
- [ ] 日本語ドキュメントを用いた検索精度の比較評価を実施する
