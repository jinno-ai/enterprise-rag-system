...
## Issue 6: 日本語テキスト処理の高度化 (形態素解析と日本語区切り文字による精度向上)

**タイトル:** 日本語形態素解析の導入によるハイブリッド検索(BM25)およびテキスト分割(Chunking)精度の向上

**内容:**
現在、`HybridRetriever` の BM25 インデックス作成において単純な正規表現（`re.findall(r'\w+', ...)`）による単語分割が使用されており、日本語テキストの分かち書きに対応できていません。また、`TextSplitter` / `DocumentChunker` の区切り文字設定に日本語の句読点（`。`, `！`, `？` など）が含まれておらず、日本語ドキュメントの文脈途中でチャンクが分断される課題があります。

これらを改善するため、日本語形態素解析ライブラリ（Janome または SudachiPy）を導入し、BM25 トークナイズおよび日本語対応テキストチャンキングの精度向上を図ります。

**タスク:**
- [ ] 日本語形態素解析ツール（Janome/SudachiPy等）を用いた BM25 用トークナイザーの実装
- [ ] `TextSplitter` / `DocumentChunker` に日本語句読点（`。`, `！`, `？`）を分割区切り文字として追加
- [ ] 日本語ドキュメントに対する検索・チャンキングの精度検証単体テストの追加
