...
**内容:**
現在、`app/services/retrieval.py` の `HybridRetriever.build_bm25_index` において、日本語テキストのトークン化に単純な正規表現（`re.findall(r'\w+', ...)`）が使用されています。これにより、日本語の検索精度（特にBM25によるキーワード検索）が著しく低下しています。日本語の検索精度を向上させるために、MeCabやSudachi、spaCyなどの形態素解析器を導入する必要があります。

**タスク:**
- [ ] 日本語形態素解析ライブラリ（例: `sudachipy`, `fugashi`）の導入
- [ ] `HybridRetriever` 内のトークナイザーを日本語対応のものに差し替え
- [ ] 日本語特有のストップワード処理の追加
- [ ] 日本語ドキュメントを用いた検索精度の評価テストの作成
