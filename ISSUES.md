...
**タイトル:** 日本語形態素解析の導入によるBM25検索精度の改善

**内容:**
現在、`app/services/retrieval.py` の `HybridRetriever` クラスにおけるBM25インデックス作成（`build_bm25_index`）およびキーワード検索（`keyword_search`）では、簡易的な正規表現 (`re.findall(r'\w+', ...)`) によるトークナイズが行われています。

これは英語などのスペース区切り言語には有効ですが、日本語のような分かち書きをしない言語においては単語を正しく分割できず、キーワード検索の精度が著しく低下します。エンタープライズレベルの検索品質を確保するため、日本語形態素解析器（Janome, SudachiPy, MeCab等）を導入し、適切なトークナイズを行う必要があります。

**タスク:**
- [ ] `requirements.txt` に日本語形態素解析ライブラリ（例: `janome`）を追加する
- [ ] `app/services/retrieval.py` に日本語対応のトークナイザー関数を実装する
- [ ] `HybridRetriever` が環境設定やテキスト内容に応じて適切なトークナイザー（デフォルト vs 日本語）を選択できるようにする
- [ ] 日本語のクエリおよびドキュメントを用いたキーワード検索の有効性を検証するテストケースを追加する
