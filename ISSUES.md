...
## Issue 6: 非同期ジョブキュー（Celery/Redis）によるドキュメント処理のバックグラウンド化

**タイトル:** 【Phase 3.1】非同期ジョブキュー（Celery/Redis）によるドキュメント処理のバックグラウンド化

**内容:**
現在、`app/api/routes/ingest.py` のインジェクション機能はモック実装となっており、また `run.py` と `document_loader.py` の間でメソッド名の不一致が発生しています。大規模なドキュメント群の処理において、APIリクエスト内での同期処理はタイムアウトのリスクがあるため、ロードマップ Phase 3.1 に基づき非同期タスク処理を導入します。

**タスク:**
- [ ] `Celery` および `Redis` を導入し、非同期タスク実行基盤を構築する
- [ ] `DocumentLoader` と `TextSplitter` を統合し、`run.py` で使用されている `load_file` や `chunk_documents` などのインターフェースを整理・実装する
- [ ] インジェクション処理を非同期タスク化し、APIから進捗状況（ステータス、処理済み件数、エラー等）を確認可能にする
