...
## Issue 6: 日本語トークナイズのサポート (BM25)

**タイトル:** `HybridRetriever` における日本語形態素解析の導入

**内容:**
現在、`app/services/retrieval.py` 内の `build_bm25_index` および `keyword_search` では、`re.findall(r'\w+', text.lower())` という簡易的な正規表現によるトークナイズが行われています。これは英語などのスペース区切り言語には有効ですが、分かち書きを行わない日本語では適切に単語を分割できず、検索精度が著しく低下します。

**タスク:**
- [ ] 日本語形態素解析ライブラリ（`SudachiPy`, `MeCab`, `Janome` 等）を導入する
- [ ] 日本語と英語が混在する環境でも適切にトークナイズできるユーティリティ関数を作成する
- [ ] `HybridRetriever` の BM25 インデックス構築および検索処理を、新しいトークナイザーを使用するように修正する
- [ ] `requirements.txt` に必要なライブラリを追加する
