...
**タイトル:** FAISSにおけるメタデータフィルタリングの実装

**内容:**
現在、`FAISSVectorDB.search` メソッドには `filter_dict` 引数が存在しますが、実装が空（無視される状態）になっています。これにより、特定のドキュメントソースやカテゴリに絞った検索ができません。

FAISSの `IndexIDMap` や、検索後のフィルタリング処理を導入することで、メタデータに基づいた絞り込み機能を実現する必要があります。

**タスク:**
- [ ] `FAISSVectorDB` においてメタデータフィルタリングのロジックを実装する
- [ ] フィルタリングが正しく動作することを確認するユニットテストを追加する
- [ ] 他のVectorDB（Pinecone等）との挙動の整合性を確認する
