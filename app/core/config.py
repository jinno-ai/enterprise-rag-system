...
# Global settings instance (will try to read from environment)
try:
    settings = Settings()
except Exception:
    # Fallback for environments where required env vars might be missing during import
    # but provided via mocks in tests
    settings = None
...
def get_settings() -> Settings:
    ...
    if settings is None:
        # Re-attempt to load or return a mockable object if needed
        return Settings(_env_file=None, openai_api_key="mock")
    return settings
