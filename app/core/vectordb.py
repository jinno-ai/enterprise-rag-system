...
class VectorDB(ABC):
    ...
    def add_documents(
        ...
        if hasattr(self, "index") and getattr(self, "index") is None:
        ...
        # Inject text into metadata if not present
        prepared_metadatas = []
...
class FAISSVectorDB(VectorDB):
    ...
    def search(
        ...
        # If filtering is needed, we might need to over-retrieve and filter manually
        # as FAISS flat index doesn't support metadata filtering natively
        actual_top_k = top_k * 10 if filter_dict else top_k
