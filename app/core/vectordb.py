...
class VectorDB(ABC):
    ...
    def add_documents(
        ...
        # Ensure document text is in metadata for retrieval
...
class FAISSVectorDB(VectorDB):
    ...
    def upsert(
        ...
        if self.index is None:
            # Auto-initialize index if it doesn't exist and we have embeddings
            if vectors:
                dimension = len(vectors[0])
                self.create_index(dimension)
            else:
                raise RuntimeError("Index not created. Call create_index() or provide vectors.")
    ...
    def search(
        ...
        # Over-sample to allow for filtering
        fetch_k = top_k if filter_dict is None else top_k * 10
