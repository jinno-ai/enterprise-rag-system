...
class FAISSVectorDB(VectorDB):
    ...
    def upsert(
        ...
        if self.index is None:
            print(f"Initializing FAISS index with dimension {vectors_np.shape[1]}")
            self.create_index(dimension=vectors_np.shape[1])
    ...
    def search(
        ...
        # If we have filters, we over-sample to ensure we find enough results
